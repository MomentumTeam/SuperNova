...
export async function areAllSubRequestsFinished(
  call: any,
  callback: any
): Promise<void> {
  try {
    logger.info(`Call to areAllSubRequestsFinished`, {
      callRequest: call.request,
    });
    const response = await requestManager.areAllSubRequestsFinished(
      call.request
    );
    logger.info(`areAllSubRequestsFinished OK`, {
      callRequest: call.request,
      response: response,
    });
    callback(null, response);
  } catch (error: any) {
    logger.error(`areAllSubRequestsFinished ERROR`, {
      callRequest: call.request,
      error: { message: error.message },
    });
    callback(
...
export async function removeApproverFromApprovers(
  call: any,
  callback: any
): Promise<void> {
  try {
    logger.info(`Call to removeApproverFromApprovers`, {
      callRequest: call.request,
    });
    const response = await requestManager.removeApproverFromApprovers(
      call.request
    );
    logger.info(`removeApproverFromApprovers OK`, {
      callRequest: call.request,
      response: response,
    });
    callback(null, response);
  } catch (error: any) {
    logger.error(`removeApproverFromApprovers ERROR`, {
      callRequest: call.request,
      error: { message: error.message },
    });
    callback(
