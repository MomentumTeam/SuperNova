...
export class MongoConn {
  connectMongo = async () => {
    ...
    try {
      await this.startConnectionAttempts();
    } catch (err) {
   ...
   */
  startConnectionAttempts = async () => {
    const retries = config.mongo.connectionRetries;
    const timeout = config.mongo.reconnectTimeout;

    for (let i = 1; i <= retries; i++) {
      const connectionRes: { success: boolean; error: Error | null } = await this.connect();
   ...
   */
  connect = async (): Promise<{ success: boolean; error: Error | null }> => {
