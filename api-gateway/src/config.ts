...
export const config = {
  ...
  fields: {
    ...
    rootId: env.get("GATEWAY_ROOT_ID").default("619e3193f235dc001846bb4f").asString(),
  ...
  endpoints: {
    ...
    supportLink: env.get("GATEWAY_SUPPORT_LINK").default("https://www.lego.com/en-us/service").asString(),
  },
  files: {
  ...
  spike: {
    ...
    publicKeyPath: env.get("GATEWAY_SPIKE_PUBLIC_KEY_FULL_PATH").default(spikePubKeyDefault).asString(),
    writeScopeName: env.get("GATEWAY_WRITE_SCOPE_NAME").default("write").asString(),
  },
  ui: {
  ...
  logs: {
    ...
    logPath: process.env.GATEWAY_LOG_PATH ? `${process.env.GATEWAY_LOG_PATH}/api-gateway` : "./logs/api-gateway",
  },
};
